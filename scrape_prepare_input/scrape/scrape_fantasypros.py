...
def get_player_season_weekly_ranks(name_php, pos, year=2019):
    ...
    for week in tqdm(range(1, 17), desc='get_player_season'):
        df_player_week, _ = scrape_FantasyPros_player(name_php, year, pos, weekly=True, week=week)
        df_player_weeks.append(df_player_week)
    print(f'Done get_player_season: {name_php}, {len(df_player_weeks)=}')
    return df_player_weeks


def get_full_season_ranks(year, n_inst=5):
    '''
    Scrapes all weekly rankings for all players (all positions) for a given year
        See get_all_seasons_weekly_ranks(...) for more info

    :param year: single year to scrape
    :param n_inst: Number of instances for parallelization
    :return: df_rankings, one row per expert per player per week
    '''
    php_players, _, pos_l = get_names_php(year)
    print(f'get_all_players({year}, n_inst={n_inst}): {len(php_players)=}')
    start = time.time()
    get_player_season_year = partial(get_player_season_weekly_ranks, year=year)
    df_all = process_map(get_player_season_year, php_players, pos_l, max_workers=n_inst)
    df_all = [df for df_season in df_all for df in df_season]
    print(f'{year}: Time needed for {len(php_players)} players, '
          f'concurrent ({n_inst} instances): {time.time() - start:.3f}s')
    return pd.concat(df_all), None


def get_all_seasons_weekly_ranks(year_st=2018, year_end=2021, n_inst=5,
                                 override=False):
    '''
    Scrapes all weekly rankings for all players from years year_st to year_end.
        Scrapes all positions.
        Scraping is structured as a hierarchy.
    This function iterates through all years.
        For each year, it calls get_full_season_ranks(year),
        which scrapes all players for a given year.
    get_full_season_ranks(year) iterates over all players.
        For each player, get_full_season_ranks(year) calls
        get_player_season_weekly_ranks(...), which scrapes a full season for
        a given player.
    ...
    for year in range(year_st, year_end):
        ...
        df_year, _ = pickle_wrap(fp_pkl,
                              lambda: get_full_season_ranks(year, n_inst=n_inst),
