...
from scrape_prepare_input.scrape.scrape_scores import get_df_scores
...
def get_df_rankings_scores(year_st, year_end):
    ...
    Loads the primary dataset. The dataset is made by loading and combining two:
        ...
        df_scores: It represents the actual scores of every player every week.
            ...
            salary, although the salary data are not used by the present code.
    df_rankings is scraped from FantasyPros.com.
        See scrape.get_all_seasons_weekly_ranks(...)
    df_scores is scraped from rotoguru1.com.
        See scrape.get_df_scores(...)
    ...
    df_rankings = get_all_seasons_weekly_ranks(year_st, year_end, n_parallel=10,
                                               override=False)
...
def pivot_experts2columns(df_rankings, groupby_code=('year', 'pos', 'Player')):
    ...
    Note: 'ECR' = Expert Consensus Ranking, which is a column from FantasyPros
        The Expert Consensus Ranking is basically an average of all the experts

    :param df_rankings:
    :param groupby_code: Specifies the categories ranked
    :return: Modified input dataframe
             and a list of all the experts, which may be helpful
    '''
